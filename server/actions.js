...
const OPENAPI = {
  ...
  paths: {
    ...
    '/actions/artikelplan': { get: { operationId: 'artikelplan', 'x-openai-isConsequential': false, summary: 'Strategisk artikelplan: 12-månaderskalender med målsökord, volymer, internlänkningsplan och AEO-vinkel. Förstahandskälla för vad som ska skrivas härnäst — stäm av mot bloggLista.', parameters: [q('manad', 'Filtrera på månad, t.ex. 2026-10'), q('prio', 'P1/P2/P3'), q('pelare', 'Pelarnamn'), q('status', 'planerad eller klar')], responses: RESP } },
    '/actions/artikelplan-markera': { post: { operationId: 'artikelplanMarkera', 'x-openai-isConsequential': true, summary: 'Markera en planartikel som klar (eller planerad igen). Sker automatiskt om plan_nr anges vid artikelSkapa.', requestBody: { required: true, content: { 'application/json': { schema: P({ nr: S.int, status: { ...S.str, description: 'klar (standard) eller planerad' }, artikel_id: S.str }, ['nr']) } } }, responses: RESP } },
    '/actions/produkt': {
...
router.get('/artikelplan', run('artikelplan', r => ({ manad: r.query.manad, prio: r.query.prio, pelare: r.query.pelare, status: r.query.status })));
router.post('/artikelplan-markera', run('artikelplanMarkera', r => r.body || {}));
