...
const OPENAPI = {
  ...
  paths: {
    ...
    '/actions/artikel': {
      ...
      post: { operationId: 'artikelSkapa', 'x-openai-isConsequential': true, summary: 'Skapa ett nytt blogginlägg i Shopify (UTKAST om inte publicera=true uttryckligen begärts av användaren). Svara användaren med adminUrl för granskning.', requestBody: { required: true, content: { 'application/json': { schema: P({ titel: S.str, html: { ...S.str, description: 'Brödtext som HTML: h2/h3/p/ul/a, ingen h1, minst 200 tecken' }, seo_titel: { ...S.str, description: 'SEO-titel, max 60 tecken, huvudsökordet tidigt — sätt ALLTID' }, seo_beskrivning: { ...S.str, description: 'Meta-beskrivning, max 155 tecken, ska locka till klick — sätt ALLTID' }, sammanfattning: S.str, taggar: S.arr, blogg_id: S.str, publicera: { ...S.bool, description: 'true endast om användaren uttryckligen bett om publicering' }, forfattare: S.str }, ['titel', 'html', 'seo_titel', 'seo_beskrivning']) } } }, responses: RESP },
    },
