...
export async function processImage(inputBuffer, options = {}) {
  ...
  if (!sharp) {
    return { webp: null, jpg: inputBuffer, metadata: { originalWidth: 0, originalHeight: 0, webpSize: 0, jpgSize: inputBuffer.length, isLowRes: false } };
  }
  ...
  const processed = sharp(inputBuffer).resize(targetSize, targetSize, { fit: 'contain', background: backgroundColor });
  ...
  if (outputFormat === 'webp' || outputFormat === 'both') {
    ...
    if (webpBuffer.length > maxFileSize) {
      webpBuffer = await sharp(webpBuffer).webp({ quality: Math.max(40, webpQuality - 20) }).toBuffer();
    }
  }
  ...
  return {
    webp: webpBuffer,
    jpg: jpgBuffer,
    metadata: { originalWidth: origWidth, originalHeight: origHeight, originalFormat: origFormat, processedWidth: targetSize, processedHeight: targetSize, webpSize: webpBuffer?.length || 0, jpgSize: jpgBuffer?.length || 0, isLowRes },
  };
}
...
export async function getImageMetadata(buffer) {
  const sharp = await getSharp();
  if (!sharp) return { width: 0, height: 0, format: 'unknown', size: buffer.length, hasAlpha: false };
  const metadata = await sharp(buffer).metadata();
  return { width: metadata.width, height: metadata.height, format: metadata.format, size: buffer.length, hasAlpha: metadata.hasAlpha };
}
