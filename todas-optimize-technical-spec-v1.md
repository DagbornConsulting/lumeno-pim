...
model Client {
  ...
  // Brand voice (synkas med Todas Client Context Engine)
  toneOfVoice      String?  @db.Text
  brandProfile     Json?    // JSON: värderingar, USP:ar, terminologi
...
### 4.4 Steg 4: AI-berikning (enrichment-service.ts)
...
export async function enrichProduct(input: EnrichmentInput): Promise<EnrichmentOutput> {
  ...
  // 1. Bygg prompt med all kontext
  const prompt = buildEnrichmentPrompt({
    rawData: product.rawData,
    brandVoice: client.toneOfVoice,
